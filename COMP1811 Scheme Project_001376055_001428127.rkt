...
(define (tr-current-age lst)
  ...
  (define current-year (date-year date))
  (define current-month (date-month date))
  (define current-day (date-day date))
  
  ;; Calculate age 
  (define (calculate-age birth-date)
    (define birth-day (car birth-date))
    (define birth-month (cadr birth-date))
    (define birth-year (caddr birth-date))
    (define year-diff (- current-year birth-year))
  ...
  (define (current-age lst acc)
    (if (null? lst) acc
        (if (is-alive? (car lst))
            (current-age (cdr lst)
             (cons (list (caar lst) ; Name
                         (calculate-age (caaddr (car lst)))) ; Age
                   acc))
         ...
         (list
          ...
          (map (λ (parent)
           ;; Change perents's name if it matches old-name
                 (if (and (pair? parent) (equal? (car parent) old-name))
